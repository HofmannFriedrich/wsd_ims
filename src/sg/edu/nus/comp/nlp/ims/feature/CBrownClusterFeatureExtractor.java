 ...
 *
 */
public class CBrownClusterFeatureExtractor implements IFeatureExtractor {
	...
	// Cluster types
	protected static CClusterExtractor.clusterTypes m_clusterType = CClusterExtractor.clusterTypes.BROWN;
	 ...
	 * constructor
	 */
	public CBrownClusterFeatureExtractor() {
	 ...
	 */
	public CBrownClusterFeatureExtractor(int p_Left, int p_Right) {
	...
	protected String formCWCName(int p_Index) {
		...
		if(p_Index<0){
			return "CWBC_" + -p_Index + "C" + this.m_IndexInEmbedding;
		}
		else
		{
			return "CWBC" + p_Index + "C" + this.m_IndexInEmbedding;
		}
	...
	private IFeature getNext() {
		...
		// Get the set of words for the current representation
	...
	public boolean setCurrentInstance(int p_Index) {
		if (this.validIndex(p_Index)) {
			...
			this.m_clusterExtractor.loadClusters(this.m_clusterType.ordinal(), this.m_BrownClusterPath);
