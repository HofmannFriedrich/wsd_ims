 ...
 *
 */
public class CClarkClusterFeatureExtractor implements IFeatureExtractor {
	...
	// Cluster types
	protected static CClusterExtractor.clusterTypes m_clusterType = CClusterExtractor.clusterTypes.CLARK;
	 ...
	 * constructor
	 */
	public CClarkClusterFeatureExtractor() {
	 ...
	 */
	public CClarkClusterFeatureExtractor(int p_Left, int p_Right) {
	...
	protected String formCWCName(int p_Index) {
		...
		if(p_Index<0){
			return "CWCC_" + -p_Index + "C" + this.m_IndexInEmbedding;
		}
		else
		{
			return "CWCC" + p_Index + "C" + this.m_IndexInEmbedding;
		}
	...
	public boolean setCurrentInstance(int p_Index) {
		if (this.validIndex(p_Index)) {
			...
			this.m_clusterExtractor.loadClusters(this.m_clusterType.ordinal(), this.m_ClarkClusterPath);
			//this.getCWESet();
